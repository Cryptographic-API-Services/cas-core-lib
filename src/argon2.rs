...
pub extern "C" fn argon2_hash_thread(pass_to_hash: *const c_char) -> *mut c_char {
    ...
    .unwrap()
    .as_bytes();
    let password_hash = thread::spawn(move || {
        let salt = SaltString::generate(&mut OsRng);
        let argon2 = Argon2::default();
        return argon2.hash_password(pass_bytes, &salt).unwrap().to_string();
    })
    .join()
    .unwrap();
    return CString::new(password_hash).unwrap().into_raw();
}
...
fn argon2_hash_thread_test() {
    ...
    let password_bytes = password_cstr.as_bytes_with_nul();
    let password_ptr = password_bytes.as_ptr() as *const i8;
    let hashed_password_ptr = argon2_hash_thread(password_ptr);
    let hashed_password_ctr = unsafe { CString::from_raw(hashed_password_ptr) };
    let hashed_password_str = hashed_password_ctr.to_str().unwrap();
    assert_ne!(password, hashed_password_str);
}
